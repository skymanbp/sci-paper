...
| `cli_common.py` | Shared command-line preamble: UTF-8 stdout, and the `--field` / `--profile-root` / `--corpus-root` options every field-aware tool takes. | Holds no policy — decides no default beyond the profile root, reads no profile, emits no findings. Tools written before 2026-08-26 keep their own copies; their parsers diverge in the options that follow, so a sweep of 26 CLIs was judged to carry more regression risk than the duplication removes. New tools use this. |
