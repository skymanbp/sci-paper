...
# Buckets: abstract / intro / method / results / discussion / conclusion / skip.
# Sections with no match default to "method" — empirically, in WGL papers most
# thematic section names ("Aperture mass", "Cosmological background",
# "How to measure shear", "Object Detection") are method / expository writing.
...
def _classify_pdf_heading(stripped: str) -> str | None:
    ...
    Two-stage detection (precision-first):
      ...
      2. ALL-CAPS heuristic: line is short (≤ 12 words), ≥ 60% uppercase
         letters, and contains no body-text punctuation (comma, semicolon,
         question mark). Catches thematic headings like
         "2 OPTIMAL FILTER FOR GALAXY" that journal-style PDFs typeset in
         caps. Unmatched-by-keyword headings classify via
         classify_section() (defaulting to "method", the catchall).
    """
