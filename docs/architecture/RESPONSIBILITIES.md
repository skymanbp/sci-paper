...
| `condense` | The redundancy/length action surface: execute §5.3 (delete > condense-in-place > same-length; growth only with recorded justification) with one-canonical-home-per-fact deduplication over the machine-built removal map (`condense_map.py`), one recorded disposition per map entry, proven by `length_gate.py --require-shrink` against the map's target and by the residue diff gate; never delete a fact's sole support. |
...
| `deai_structure.py` | L2 sentence | Emit template evidence with calibration metadata; advisory-success exit 0. The auxiliary families (antithesis, reversal beat, paper-as-agent, wh-cleft, modifier stack) name the sentence under `structure-auxiliary` and never enter `template_score`. |
...
| `deai_register.py` | L0 register | Compare manuscript vocabulary against field-corpus document frequency; judge compounds by their rarest part; read macro bodies but not subscript decorations; emit advisories only, never `l0_target`s. The zero-hit audit (`register-zero:<term>`) is exhaustive over body words on both sides of the same projection (headings, comments, preamble and bibliography excluded on both), strong unless the word is a mechanical formation of an attested stem. |
| `deai_collocation.py` | L2 collocation | Judge, per sentence, the fraction of adjacent common-word pairs (unigram df at or above `COMMON_RATE`) that no corpus passage attests, against a leave-one-out per-section reference at sentence unit; a pair breaks at punctuation, a placeholder or a dash; carry each pair's chance-absence probability so a reader can weigh it; advisories only, and the action names the relation, never the claim. Read document-wide (`--glossary`), list every unattested pair used twice or more with its first-use line and definition cue, as the manuscript's own terms. |
| `deai_residue.py` | L4 residue | Report self-referential drafting history (first-person sentence, no citation), edit-meta literals, and a heading or caption whose object the body never names; with `--before`/`--git-ref`, the added-label diff rule; exit 1 on a strong finding (§0.1); keep the family lists in one place and let the validator prove the skills quote them. |
| `condense_map.py` | QD (§5.3) | Enumerate every removable entry in six scans, each with `removable_words` and, for a restatement, its canonical home; total a default target that excludes the abstract/conclusion carve-out; never delete anything, never emit a blocker, exit 0/2. |
...
| `eval_findings.py` | evidence | Score the register, salience, cohesion, hedging and collocation axes (the last on its document novel-pair fraction, because its reference is per sentence) on a **held-out** refereed corpus that fed no calibration bank, and report the same-genre in-sample population beside it so the leakage gap stays visible. Never present the salience rate as a false-positive rate — its gate is a percentile, so a non-zero rate is its design point — and never present either rate as precision for the advice itself. Added 2026-08-27: the axes' precision had stood at `unmeasured` pending hand labels, while a refereed paper's provenance was already a label for half of the question. |
