...
| Human-labelled validation for salience and register | **Open — blocked on labels** | The only remaining open item. The harness is built (`tools/label_findings.py`): finding-level labels, one labeller plus a blind re-label subset, drafts and published papers stratified. It calibrates nothing until labels exist, and reports `unmeasured` below 20 per stratum. |
