# EVALUATION: de-AI subsystem for `sci-paper` v0.30.1
...
| L0 register | measured | §14: document frequency over 41,593 corpus passages, 55,133 terms; compound-by-rarest-part and macro-subscript handling; native-term controls pass. | Recall below the 5-use floor, and now a **measured** false-positive rate: §17.4 puts it at 0.991 findings per 1,000 words on 203 held-out refereed papers (93.6% of documents), with rank AUC 0.080 against machine text — it fires more on human prose than on AI drafts. §17.3: 72.7% of those flags would be suppressed by the paper's own bank membership, so the in-sample view was ~3.7× optimistic. The operating point needs re-deriving against a held-out target rate. Since v0.30.1 a bank whose resolution (1/n) cannot express the 1e-4 gate reports `degraded` naming the coarseness rather than `measured`: `wgl-letter` at 706 passages was 14.2x coarser and had been running 'df == 0' as its rule. |
...
The working tree passes the validator and all 308 unit/CLI tests (17 test files, collected
...
Current release gates (v0.30.1, 2026-08-27): `validate_plugin.py` all 9 checks
pass and the full unit/CLI suite (308 tests, 17 files) passes on a clean tree;
