# EVALUATION: de-AI subsystem for `sci-paper` v0.36.2
...
| L2 collocation | measured | §23.2: per-sentence fraction of adjacent common-word pairs unattested in 41,710 passages, leave-one-out reference per bucket at sentence unit (abstract 89,392; method 27,293; intro 14,382; results 12,419; data 12,054; discussion 12,205; conclusion 8,571 after v0.36.2's pair breaks at slashes and digits); p90 gates 0.44–0.59, p95 0.50–0.67. On the first measured draft of the private Letter it flagged five of the six mentor-marked phrases present; the document novel-pair fraction separates machine documents from held-out refereed papers at AUC 0.704. | Whether an individual advisory is good advice; a second generation process (§20's lesson). |
| L4 residue | measured | §23.4: three static rules and one diff rule, deterministic, strengths set on 203 held-out refereed papers (70,848 body prose sentences): strong self-history fires in 20 papers, strong edit-meta in 8 (68 of 69 are `\textcolor{red}` in tables), any strong in 27 (13.3%); the static negative-label rule is ordinary at 26% of papers. Four passes to get there, the first a projection asymmetry and the last the procedural sense of `we have added`. | Recall against a labelled edit history; only the author's own drafts can supply it. |
...
The working tree passes the validator and all 491 unit/CLI tests (24 test files, collected
...
(491 tests, 24 files) passes on a clean tree; both are rerun before every tag,
